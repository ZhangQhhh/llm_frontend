  ...
  <div :class="['exam-page', { 'light-mode': isLightMode }]">
    ...
    <div :class="['wrap', { 'low-perf-mode': isLowPerformanceMode }]">
      ...
      <div class="side card" v-if="examStarted && questions.length > 0 && !isLowPerformanceMode">
        ...
        <!-- 判断题导航 -->
        <div v-if="judgeQuestions.length > 0" class="nav-section">
          <div class="nav-section-title">
            <span class="nav-type-tag judge">判断</span>
            <span class="nav-count">{{ judgeQuestions.length }}题</span>
          </div>
          <div class="navgrid">
            <button
      ...
      <div class="main">
        ...
        <div class="card head">
          ...
          <div class="sub" v-if="examStarted && questions.length > 0">
            ...
            <span class="head-divider" v-if="indeterminateQuestions.length > 0 && (judgeQuestions.length > 0 || saqQuestions.length > 0)">|</span>
            <span class="head-stat" v-if="judgeQuestions.length > 0">
              <span class="stat-label">判断</span>
              <span class="stat-value">{{ judgeQuestions.length }}题</span>
            </span>
            <span class="head-divider" v-if="judgeQuestions.length > 0 && saqQuestions.length > 0">|</span>
        ...
        <div v-else-if="!reviewData" class="qlist">
              ...
              <div v-if="visibleStemImagesOf(q).length > 0" class="stem-images">
                <template v-if="isLowPerformanceMode && !loadedImages[q.qid + '_stem']">
                  <div class="image-placeholder" @click="loadedImages[q.qid + '_stem'] = true">
                    📷 点击加载图片 ({{ visibleStemImagesOf(q).length }}张)
                  </div>
                </template>
                <template v-else>
                  <img
          ...
          <!-- 判断题区域（单选题特例：2 选项 正确/错误） -->
          <div v-if="visibleJudgeQuestions.length > 0" class="question-section">
            <div class="section-header">
              <span class="section-tag judge">{{ getJudgeSectionNumber() }}、判断题</span>
              <span class="section-count">{{ isLowPerformanceMode ? `本页 ${visibleJudgeQuestions.length} 题` : `共 ${judgeQuestions.length} 题` }}</span>
            </div>
            <div v-for="(q, idx) in visibleJudgeQuestions" :key="q.qid" :id="'q-' + q.qid" class="q">
              <div class="qheader">
              ...
              <div class="opts" v-if="q.options && q.options.length">
                <button
                  ...
                  :class="['opt', { active: answersState[q.qid] === opt.label }]"
                  :disabled="submitted"
                  @click="!submitted && ((answersState[q.qid] = (answersState[q.qid] === opt.label ? '' : opt.label)), debounceSave())"
                >
            ...
            <div v-for="(item, idx) in sortedReviewItems" :key="idx" :id="'review-q-' + item.qid" class="q">
              <div class="qheader">
                ...
                <span :class="['tag', item.qtype]">
                  {{ item.qtype === 'multi' ? '多选' : (item.qtype === 'indeterminate' ? '不定项' : (item.qtype === 'judge' ? '判断' : (item.qtype === 'saq' ? '简答' : '单选'))) }}
                </span>
              </div>
              <!-- 题干图片（表格内嵌入的图片在表格内渲染，不重复显示） -->
              <div v-if="visibleStemImagesOf(item).length > 0" class="stem-images">
                <img
              ...
              <div class="analysis" @mouseenter="isComplexValidation(item.analysis) && loadPerOption(item.qid)">
                <template v-if="isComplexValidation(item.analysis)">
                  ...
                  <div class="stem-html" v-html="formatAnalysis(getAnalysisForTab(item.qid, item.analysis, analysisActiveTab[idx] || 'all'), item) || '（无解析）'" @click="onStemContentClick"></div>
                </template>
                <template v-else>
                  <div class="stem-html" v-html="formatAnalysis(item.analysis, item) || '（无解析）'" @click="onStemContentClick"></div>
                </template>
                <!-- 解析图片 -->
      ...
      <div class="random-practice-dialog">
        <div class="practice-config-dialog">
          ...
          <div class="config-row-dialog">
            ...
            <div class="config-item-dialog">
              <label>判断题数量</label>
              <el-input-number v-model="practiceConfig.judgeCount" :min="0" :max="50" size="default" />
            </div>
          </div>
          <div class="config-row-dialog">
            <div class="config-item-dialog">
        ...
        <div v-else class="wrong-list">
          <div v-for="(q, idx) in wrongBook" :key="q.qid" class="wrong-item">
            <div class="wrong-header">
              ...
              <span class="wrong-stem stem-html" v-html="formatText(q.stem, q)" @click="onStemContentClick"></span>
              <el-tag size="small" :type="q.qtype === 'multi' ? 'danger' : (q.qtype === 'saq' ? 'info' : (q.qtype === 'judge' ? 'success' : 'primary'))">
                {{ q.qtype === 'multi' ? '多选' : (q.qtype === 'indeterminate' ? '不定项' : (q.qtype === 'judge' ? '判断' : (q.qtype === 'saq' ? '简答' : '单选'))) }}
              </el-tag>
              <el-button 
                size="small" 
                type="success" 
            ...
            <!-- 题干图片（表格内嵌入的图片在表格内渲染，不重复显示） -->
            <div v-if="visibleStemImagesOf(q).length > 0" class="wrong-images">
              <img
                v-for="(img, imgIdx) in visibleStemImagesOf(q)"
                :key="'stem-' + imgIdx"
                :src="'data:' + img.content_type + ';base64,' + img.base64"
            ...
            <div v-if="q.explain" class="wrong-explain">
              ...
              <div class="stem-html" v-html="formatAnalysis(q.explain, q)" @click="onStemContentClick"></div>
              <!-- 解析图片 -->
...
import { formatTextWithTables, buildQuestionImagesMap, visibleStemImages, renderTableBlocks } from '@/utils/tableRender'
...
export default defineComponent({
  ...
  setup() {
    ...
    const practiceTotalCount = computed(() => {
      return practiceConfig.singleCount + practiceConfig.multiCount + practiceConfig.indeterminateCount + practiceConfig.judgeCount + practiceConfig.saqCount
    })

    // 检测是否为判断题选项（恡76项，内容为正确/错误等对偶词对）
    const _JUDGE_OPTION_PAIRS: Set<string>[] = [
      new Set(['正确', '错误']), new Set(['对', '错']), new Set(['是', '否']),
      new Set(['true', 'false']), new Set(['t', 'f']), new Set(['yes', 'no']), new Set(['y', 'n']),
      new Set(['√', '×']), new Set(['对的', '错的']), new Set(['正确的', '错误的']),
    ]
    const _isJudgeOptions = (options: any): boolean => {
      try {
        let texts: string[] = []
        if (Array.isArray(options)) {
          texts = options.map((o: any) => (o?.text ?? o ?? '').toString().trim()).filter(Boolean)
        } else if (options && typeof options === 'object') {
          texts = Object.values(options).map((v: any) => (v ?? '').toString().trim()).filter(Boolean)
        }
        if (texts.length !== 2) return false
        const norm = new Set(texts.map(t => t.toLowerCase().replace(/[。.!！？?·、 ]+$/g, '')))
        return _JUDGE_OPTION_PAIRS.some(pair => {
          if (pair.size !== norm.size) return false
          for (const v of pair) if (!norm.has(v)) return false
          return true
        })
      } catch { return false }
    }
    const _isJudgeQuestion = (q: any): boolean => {
      if (!q) return false
      if (q.qtype === 'judge') return true
      return _isJudgeOptions(q.options)
    }
    ...
    // 按题型排序的解析列表（单选→多选→不定项→判断→简答）
    const sortedReviewItems = computed(() => {
      if (!reviewData.value) return []
      const typeOrder: Record<string, number> = { single: 0, multi: 1, indeterminate: 2, judge: 3, saq: 4 }
    ...
    // 判断题列表（单选题特例：2 个选项 + 正确/错误）
    const judgeQuestions = computed(() => {
      return questions.value.filter(q => q.qtype === 'judge')
    })
    ...
    const visibleJudgeQuestions = computed(() => {
      if (!isLowPerformanceMode.value) return judgeQuestions.value
      return pagedQuestions.value.filter(q => q.qtype === 'judge')
    })
    ...
    const getSaqSectionNumber = () => {
      ...
      if (singleQuestions.value.length > 0) num++
      if (multiQuestions.value.length > 0) num++
      if (judgeQuestions.value.length > 0) num++  // judge 章节
      if (indeterminateQuestions.value.length > 0) num++
    ...
    const formatAnalysis = (text: string | undefined | null, q?: any): string => {
      if (!text) return ''
      // 带选项字母的进度提示替换为选项分隔标记
      ...
      let cleaned = text
        ...
        .replace(/^\s*\*\*\s*$/gm, '')
        .replace(/\*\*(?=\s*$)/gm, '')
      // 处理 [表格]...[/表格]：替换为 <table> HTML 占位符，剥掉私有区字符后交给 markdown 渲染
      const imagesMap = q ? buildQuestionImagesMap(q) : undefined
    ...
    const startRandomPractice = async () => {
      ...
      try {
        ...
        // 简答题（兼容历史数据：qtype 缺失或被误填时，按"无选项"兜底判定为 SAQ）
        const _hasOptions = (q: any): boolean => {
        ...
        const _isSaqQuestion = (q: any): boolean => {
          if (!q) return false
          if (q.qtype === 'saq') return true
          // 兜底：无任何选项的题目视为简答题
          if (!_hasOptions(q)) return true
          return false
        }
        const saqQuestions = approvedQuestions.filter((q: any) => _isSaqQuestion(q))
        // 判断题（选项为正确/错误对偶 或 qtype===judge）从选择题中剩离
        const nonSaq = approvedQuestions.filter((q: any) => !_isSaqQuestion(q))
        const judgeQuestionsPool = nonSaq.filter((q: any) => _isJudgeQuestion(q))
        const mcqQuestions = nonSaq.filter((q: any) => !_isJudgeQuestion(q))
        ...
        // 判断题（从判断题专用池抽取）
        const selectedJudge = shuffle(judgeQuestionsPool).slice(0, practiceConfig.judgeCount)
        
        // 不定项题目：从单选和多选中各抽一部分（判断题已剩离，不会混入）
        ...
        const actualJudgeCount = selectedJudge.length
        const actualSaqCount = selectedSaq.length
        const actualTotal = actualSingleCount + actualMultiCount + actualIndeterminateCount + actualJudgeCount + actualSaqCount
        ...
        // 添加判断题
        selectedJudge.forEach((q: any) => {
          paperItems.push(buildPaperItem(q, 'judge'))
        })
    ...
    return {
      ...
      visibleJudgeQuestions,
      judgeQuestions,
      visibleSaqQuestions,
      sortedQuestions,
      getSaqSectionNumber,
      getJudgeSectionNumber,
