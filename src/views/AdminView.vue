              ...
              <div class="toolbar-section filter-section">
                ...
                <div class="filter-controls">
                  ...
                  <div class="filter-right">
                    <el-button
                      type="primary"
                      @click="addNewBankQuestion"
                      :loading="addingBankQuestion"
              ...
              <el-card v-for="(q, idx) in pagedQuestions" :key="q.qid" class="question-card" shadow="hover">
                <div class="q-header">
                  <div style="display: flex; align-items: center; gap: 10px; flex: 1;">
                    ...
                    <span><strong>{{ (idx + 1) + (page-1)*pageSize }}.</strong> <span class="stem-html" v-html="formatStem(q.stem, q)" @click="onStemContentClick"></span></span>
                ...
                <!-- 题干图片（表格内嵌入的图片不重复展示） -->
                <div v-if="visibleStemImagesOf(q).length > 0" class="q-stem-images">
                  <img
                    v-for="(img, imgIdx) in visibleStemImagesOf(q)"
                    :key="imgIdx"
                ...
                <div class="q-options">
                  <div v-for="opt in q.options" :key="opt.label" class="q-option-item">
                    <span>{{ opt.label }}. <span v-html="formatStem(opt.text)"></span></span>
                    <!-- 选项图片 -->
                    ...
                    <el-form-item label="题干图片">
                      <div class="edit-images-grid">
                        <div v-for="(img, imgIdx) in editBuf.stem_images" :key="imgIdx" class="edit-image-item">
                          ...
                          />
                          <el-tag
                            v-if="extractImageMarkerFilenames(editBuf.stem).includes(img.filename)"
                            size="small" type="warning" effect="dark"
                            style="position: absolute; top: 2px; left: 2px; font-size: 10px; padding: 0 4px; height: 18px; line-height: 18px;"
                          >📊 表格内</el-tag>
                          <el-button
                            type="primary"
                            :icon="Refresh"
                            circle
                            size="small"
                            class="edit-image-replace"
                            @click="triggerReplaceStemImage(Number(imgIdx))"
                            title="替换此图片（保留原文件名，[IMG:xxx] 引用不变）"
                          />
                          <el-button
                            type="danger"
                            :icon="Close"
                            ...
                            title="删除此图片（若引用在题干 [表格] 块中，会同步移除 [IMG:xxx] 标记）"
                          />
                        </div>
                            ...
                            <div class="edit-images-grid">
                              ...
                              >
                                ...
                                <el-button
                                  ...
                                  size="small"
                                  class="edit-image-replace"
                                  @click="triggerReplaceOptionImage(k, Number(imgIdx))"
                                  title="替换此图片"
                                />
                                <el-button
                                  type="danger"
                      ...
                      <div class="edit-images-grid">
                        <div v-for="(img, imgIdx) in editBuf.analysis_images" :key="imgIdx" class="edit-image-item">
                          ...
                          <el-button
                            ...
                            size="small"
                            class="edit-image-replace"
                            @click="triggerReplaceAnalysisImage(Number(imgIdx))"
                            title="替换此图片"
                          />
                          <el-button
                            type="danger"
                ...
                <div v-if="showingAnalysis[q.qid]" class="q-analysis">
                  ...
                  <template v-if="isComplexValidation(q.analysis)">
                    ...
                    <!-- 根据Tab显示对应解析内容 -->
                    <div class="q-analysis-text stem-html" v-html="processAnalysisText(getAnalysisForTab(q.qid, q.analysis, analysisActiveTab[q.qid] || 'all'), q)" @click="onStemContentClick">
                    </div>
                  ...
                  <!-- 简单查找策略：直接显示全部 -->
                  <template v-else>
                    <div class="q-analysis-text stem-html" v-html="processAnalysisText(q.analysis, q)" @click="onStemContentClick">
                    </div>
            ...
            <div v-else class="questions-list">
              ...
              >
                ...
                <div style="margin: 10px 0;">
                  <div><strong>题目：</strong><span class="stem-html" v-html="formatStem(q.stem, q)" @click="onStemContentClick"></span></div>
                  <!-- 题干图片（表格内嵌入的图片不重复展示） -->
                  <div v-if="visibleStemImagesOf(q).length > 0" class="q-stem-images">
                    <img
                      v-for="(img, imgIdx) in visibleStemImagesOf(q)"
                  ...
                  <div class="q-options">
                    <div v-for="opt in q.options" :key="opt.label">
                      {{ opt.label }}. <span v-html="formatStem(opt.text)"></span>
                      <!-- 选项图片 -->
                      <div v-if="opt.images && opt.images.length > 0" class="q-option-images">
                        <img
                          v-for="(img, imgIdx) in opt.images"
              ...
              <el-form label-width="100px" style="margin-bottom: 16px;">
                ...
                <el-form-item label="分数设置" style="margin-bottom: 12px;">
                  <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
                    ...
                    <span>
                      <span style="margin-right: 4px;">判断题</span>
                      <el-input-number v-model="judgeScore" :min="0" :max="100" :precision="1" :step="0.1" size="small" style="width: 80px;" />
                      <span style="margin-left: 4px;">分/题</span>
                    </span>
                ...
                <el-form-item v-if="paperGenerateMode === 'manual'" label="不定项题" style="margin-bottom: 12px;">
                  ...
                  <template v-if="enableIndeterminate">
                    ...
                    <template v-else>
                      ...
                      <el-input-number v-model="indeterminateSingleCount" :min="0" :max="99" :value-on-clear="0" size="small" style="width: 90px;" />
                      <span style="margin: 0 8px;">题，多选</span>
                      <el-input-number v-model="indeterminateMultiCount" :min="0" :max="99" :value-on-clear="0" size="small" style="width: 90px;" />
                ...
                >
                  ...
                  <div class="paper-question-content">
                    <div class="paper-question-stem">
                      <el-tag :type="q.qtype === 'saq' ? 'primary' : (q.qtype === 'judge' ? 'success' : (isMultiChoice(q) ? 'warning' : 'info'))" size="small" style="margin-right: 8px;">
                        {{ q.qtype === 'saq' ? '简答' : (q.qtype === 'judge' ? '判断' : (isMultiChoice(q) ? '多选' : '单选')) }}
                      </el-tag>
                      <el-tag
                      ...
                      <span class="paper-q-num">{{ idx + 1 + (paperQuestionPage - 1) * paperQuestionPageSize }}.</span>
                      <span class="stem-html paper-stem-html" v-html="formatStem(q.stem, q)" @click="onStemContentClick"></span>
                    </div>
                    <!-- 题干图片（表格内嵌入的图片不重复展示） -->
                    <div v-if="visibleStemImagesOf(q).length > 0" class="q-stem-images" style="margin: 6px 0;">
                      <img
                        v-for="(img, imgIdx) in visibleStemImagesOf(q)"
                        :key="imgIdx"
                        :src="'data:' + img.content_type + ';base64,' + img.base64"
              ...
              <template #header>
                <div style="display: flex; justify-content: space-between; align-items: center;">
                  ...
                  <div>
                    ...
                    <el-button size="small" :icon="Refresh" @click="loadPaperList(true)" :loading="loadingPaperList">刷新</el-button>
                ...
                <el-form :inline="true" style="margin-top: 8px;">
                  <el-form-item label="分数设置">
                    <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
                      ...
                      <span style="display: flex; align-items: center;">
                        <span style="margin-right: 6px;">判断题</span>
                        <el-input-number v-model="uploadedJudgeScore" :min="0" :max="100" :precision="1" size="small" style="width: 100px;" />
                        <span style="margin-left: 4px;">分</span>
                      </span>
                      <span style="display: flex; align-items: center; gap: 8px;">
              ...
              <div style="max-height: 500px; overflow-y: auto;">
                ...
                >
                  ...
                  <div v-if="editingPaperItemIdx !== idx" class="preview-content">
                    <div class="preview-stem stem-html" v-html="item.stem ? formatStem(item.stem, item) : '（题干为空）'" @click="onStemContentClick"></div>
                    <!-- 题干图片（表格内嵌入的图片不重复展示） -->
                    <div v-if="visibleStemImagesOf(item).length > 0" class="q-stem-images" style="margin: 6px 0;">
                      <img
                        v-for="(img, imgIdx) in visibleStemImagesOf(item)"
                        :key="imgIdx"
                        :src="'data:' + img.content_type + ';base64,' + img.base64"
                        :alt="'题干图片' + (imgIdx + 1)"
                        class="q-image"
                    ...
                    <el-form label-width="60px" size="small">
                      ...
                      <el-form-item label="题干">
                        ...
                        <div style="margin-top: 8px;">
                          ...
                          <span style="color: #c0c4cc; font-size: 11px; margin-left: 8px;">（标有"📊 表格内"的图片删除后会同步移除题干 [IMG:xxx] 引用）</span>
                          <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 4px; align-items: center;">
                            <div v-for="(img, imgIdx) in (item.stem_images || [])" :key="imgIdx" class="editable-image-wrapper">
                              ...
                              />
                              <el-tag
                                v-if="extractImageMarkerFilenames(item.stem).includes(img.filename)"
                                size="small" type="warning" effect="dark"
                                style="position: absolute; top: 2px; left: 2px; font-size: 10px; padding: 0 4px; height: 18px; line-height: 18px;"
                              >📊 表格内</el-tag>
                              <el-button class="img-replace-btn" type="primary" size="small" circle :icon="Refresh" @click="triggerReplaceUploadedItemImage(Number(idx), 'stem', Number(imgIdx))" title="替换此图片（保留原文件名，[IMG:xxx] 引用不变）" />
                            ...
                            <div v-if="item.option_images && item.option_images[k] && item.option_images[k].length > 0" style="margin-left: 24px; margin-top: 4px; display: flex; flex-wrap: wrap; gap: 4px; align-items: center;">
                              <div v-for="(img, imgIdx) in item.option_images[k]" :key="imgIdx" class="editable-image-wrapper">
                                <img
                                  ...
                                  @click="previewImage('data:' + img.content_type + ';base64,' + img.base64)"
                                />
                                <el-button class="img-replace-btn" type="primary" size="small" circle :icon="Refresh" @click="triggerReplaceUploadedItemImage(Number(idx), 'option', Number(imgIdx), k)" title="替换此图片" />
                          ...
                          <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 4px; align-items: center;">
                            <div v-for="(img, imgIdx) in (item.analysis_images || [])" :key="imgIdx" class="editable-image-wrapper">
                              <img
                                ...
                                @click="previewImage('data:' + img.content_type + ';base64,' + img.base64)"
                              />
                              <el-button class="img-replace-btn" type="primary" size="small" circle :icon="Refresh" @click="triggerReplaceUploadedItemImage(Number(idx), 'analysis', Number(imgIdx))" title="替换此图片" />
            ...
            <div v-else-if="selectedExportExam && gradesStats" class="grades-stats-panel">
              ...
              <el-card v-if="topWrongQuestions && topWrongQuestions.length > 0" shadow="hover" style="margin-top: 20px;">
                ...
                <div class="wrong-question-chart">
                  <div v-for="(q, idx) in topWrongQuestions" :key="q.qid" class="wrong-question-item">
                    ...
                    <el-tag class="wq-tag" size="small"
                      :type="q.qtype === 'saq' ? 'warning' : (q.qtype === 'multi' ? 'success' : (q.qtype === 'indeterminate' ? 'danger' : (q.qtype === 'judge' ? 'info' : 'primary')))"
                      :style="q.qtype === 'judge' ? 'background:#9c27b0;color:#fff;border-color:#9c27b0;' : ''">
              ...
              <el-card shadow="hover" style="margin-top: 20px;">
                ...
                <el-table :data="pagedGradesDetails" border stripe max-height="400" style="width: 100%">
                  ...
                  <el-table-column v-if="gradesStats.has_judge" prop="judge_score" label="判断题" width="100" sortable>
                    <template #default="scope">
                      {{ scope.row.judge_score?.toFixed(1) || 0 }}
                    </template>
                  </el-table-column>
      ...
      <el-dialog v-model="questionDetailDialogVisible" title="错题详情" width="800px">
        ...
        <template v-else-if="questionDetailData">
          ...
          <div class="question-detail-content">
            <div class="qd-stem">
              <strong>题干：</strong><span class="stem-html" v-html="formatStem(questionDetailData.question?.stem, questionDetailData.question)" @click="onStemContentClick"></span>
            </div>
            <div v-if="questionDetailData.question?.qtype !== 'saq'" class="qd-options">
              <div v-for="opt in questionDetailData.question?.options" :key="opt.label" class="qd-option" :class="{ correct: questionDetailData.question?.answer?.includes(opt.label) }">
                ...
                <span class="opt-text" v-html="formatStem(opt.text)"></span>
          ...
          <el-form label-width="100px" size="default">
            ...
            <el-form-item label="修改后总分">
              <span style="font-size: 18px; font-weight: 600; color: #409eff;">
                {{ ((scoreEditForm.mcq_score || 0) + (scoreEditForm.judge_score || 0) + (scoreEditForm.saq_score || 0)).toFixed(1) }}
              </span>
            </el-form-item>
          </el-form>
        </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    // 试卷题目选择相关
    const paperQuestionFilter = ref<'all' | 'single' | 'multi' | 'judge' | 'saq'>('all')
    ...
    const randomJudgeCount = ref(0)  // 判断题数量
    ...
    const singleApprovedCount = computed(() => {
      return approvedQuestions.value.filter(q => q.qtype !== 'saq' && q.qtype !== 'judge' && !isMultiChoice(q)).length
    })
    const multiApprovedCount = computed(() => {
      return approvedQuestions.value.filter(q => q.qtype !== 'saq' && q.qtype !== 'judge' && isMultiChoice(q)).length
    })
    const judgeApprovedCount = computed(() => {
      return approvedQuestions.value.filter(q => q.qtype === 'judge').length
    })
    ...
    const uploadedJudgeScore = ref(1)
    ...
    const getQtypeLabel = (qtype: string) => {
      const map: Record<string, string> = { single: '单选', multi: '多选', indeterminate: '不定项', judge: '判断', saq: '简答' }
    ...
    const qtypeLabelMap: Record<string, string> = { single: '单选', multi: '多选', indeterminate: '不定项', judge: '判断', saq: '简答' }
    ...
    const filteredPaperQuestions = computed(() => {
      ...
      if (paperQuestionFilter.value === 'single') {
        result = result.filter(q => q.qtype !== 'saq' && q.qtype !== 'judge' && !isMultiChoice(q))
      } else if (paperQuestionFilter.value === 'multi') {
        result = result.filter(q => q.qtype !== 'saq' && q.qtype !== 'judge' && isMultiChoice(q))
      } else if (paperQuestionFilter.value === 'judge') {
        result = result.filter(q => q.qtype === 'judge')
    ...
    const uploadQuestions = async () => {
      ...
      try {
        ...
        const mcqPayload = items.map((x: any) => {
          ...
          // 透传后端识别到的章节标题/知识点
          if (Array.isArray(x.knowledge_points) && x.knowledge_points.length > 0) {
            item.knowledge_points = x.knowledge_points
          }
        ...
        // 4-pre）对"重复题目但本次解析出了章节标题/知识点"的情况，
        // 为已有记录补 knowledge_points（仅当现有记录该字段为空时补，避免覆盖人工标注）。
        // 这样无论是否走 upsert 新增，旧题都能从最新 docx 自动获得章节归属。
        try {
          const kpPatchPayload: any[] = []
          for (const d of (checkData.duplicates || [])) {
            const newKp = Array.isArray(d?.new_item?.knowledge_points) ? d.new_item.knowledge_points : []
            const existedKp = Array.isArray(d?.existing_item?.knowledge_points) ? d.existing_item.knowledge_points : []
            const existedId = d?.existing_id || d?.existing_item?.id
            if (existedId && newKp.length > 0 && existedKp.length === 0) {
              kpPatchPayload.push({
                id: existedId,
          ...
          if (kpPatchPayload.length > 0) {
            uploadMessage.value = `为 ${kpPatchPayload.length} 道已有题目补充章节标题…`
            const patchRes = await fetch(`${MCQ_BASE_URL}/bank/bulk_update`, {
              method: 'POST',
              headers: getAuthHeaders(),
              body: JSON.stringify({ items: kpPatchPayload, bank_id: currentBankId.value }),
            })
            const patchData = await patchRes.json()
            if (patchData?.ok) {
              console.log(`[Upload] 已为 ${kpPatchPayload.length} 道重复题目回填 knowledge_points`)
            } else {
              console.warn('[Upload] 回填 knowledge_points 失败:', patchData?.msg)
            }
          }
        } catch (e) {
          console.warn('[Upload] 章节标题回填异常（已忽略，不影响后续保存）:', e)
        }
        ...
        // 6）映射成前端 Question 时，记得带上 answer、图片以及结构化字段
        ...
        // 全量刷新题库列表：bulk_upsert 仅返回新增/更新的题目，
    ...
    // 懒加载当前页含图片题目的图片数据（题库管理 + 试卷管理两个 tab 的当页）
    const loadPageImages = async () => {
      const needLoad = (q: any) => q.has_images && (!q.stem_images || q.stem_images.length === 0)
      const qidSet = new Set<string>()
      for (const q of pagedQuestions.value) if (needLoad(q)) qidSet.add(q.qid)
      for (const q of pagedPaperQuestions.value) if (needLoad(q)) qidSet.add(q.qid)
      const qidsWithImages = Array.from(qidSet)
    ...
    // 新增空白题目到当前题库（题库管理）
    const addingBankQuestion = ref(false)
    const addNewBankQuestion = async () => {
      if (!currentBankId.value) {
        ElMessage.warning('请先选择或新建题库')
        return
      }
      addingBankQuestion.value = true
      try {
        const newItem = {
          stem: '',
        ...
        const r = await fetch(`${MCQ_BASE_URL}/bank/bulk_upsert`, {
          method: 'POST',
          headers: getAuthHeaders(),
          body: JSON.stringify({ items: [newItem], bank_id: currentBankId.value }),
        })
        const j = await r.json()
        if (!j?.ok) throw new Error(j?.msg || '新增失败')
        const created = (j.items && j.items[0]) || null
        if (!created) throw new Error('后端未返回新题数据')

        const newRow: any = {
          qid: String(created.id ?? created.qid ?? ''),
          stem: created.stem || '',
          options: normalizeOptions(created.options || {}, created.option_images || {}),
          answer: (created.answer || '').toString().toUpperCase(),
          analysis: created.explain || '',
          status: created.status || 'none',
          qtype: created.qtype || 'single',
          category: created.category || '',
          reference_answer: created.reference_answer || '',
          ai_generated_answer: Boolean(created.ai_generated_answer),
          has_images: Boolean(created.has_images),
          stem_images: created.stem_images || [],
          analysis_images: created.analysis_images || [],
          knowledge_clauses: created.knowledge_clauses || [],
          knowledge_points: created.knowledge_points || [],
        }
        // 插到列表最前面，并跳到第 1 页，便于用户立即看到
        questions.value = [newRow, ...questions.value]
        page.value = 1
        // 刷新题库题目计数
        loadBanksList()
        // 立即进入编辑模式
        await nextTick()
        editRow(newRow)
        ElMessage.success('已新增题目，请在上方编辑并保存')
      } catch (e: any) {
        ElMessage.error(`新增失败：${e?.message || e}`)
      } finally {
        addingBankQuestion.value = false
      }
    }
    ...
    const saveRow = async (row: any) => {
      ...
      try {
        ...
        const optionTexts = Object.values(cleanedOptions).map((v: any) => String(v || '').trim()).filter(Boolean)
        const _judgePairs = [['正确','错误'],['对','错'],['是','否'],['true','false'],['yes','no'],['√','×']]
        const isJudgeOpts = optionTexts.length === 2 && _judgePairs.some(p => {
          const norm = optionTexts.map(t => t.toLowerCase().replace(/[\s。.！？·、]+/g, ''))
          return (norm[0] === p[0] && norm[1] === p[1]) || (norm[0] === p[1] && norm[1] === p[0])
        })
        const newQtype = hasOptions
          ? (isJudgeOpts ? 'judge' : (((editBuf.answer || '').length > 1) ? 'multi' : 'single'))
          : 'saq'
        const itemData: any = {
          id: row.qid,
          stem: editBuf.stem,
          ...
          qtype: newQtype,
          status: finalExplain.trim() ? ((row.status && row.status !== 'none') ? row.status : 'draft') : 'none',
    ...
    const removeStemImage = (imgIdx: number) => {
      if (editBuf.stem_images && editBuf.stem_images.length > imgIdx) {
        const removedFn = editBuf.stem_images[imgIdx]?.filename
        editBuf.stem_images.splice(imgIdx, 1)
        // 若删除的是表格内嵌图片（题干文本中存在 [IMG:filename] 引用），同步移除文本标记
        if (removedFn && typeof editBuf.stem === 'string') {
          editBuf.stem = removeImageMarker(editBuf.stem, removedFn)
        }
      }
    }
    ...
    // ====== 编辑模式下：替换图片状态 ======
    // 记录待替换图片的索引/位置；为 null 表示新增模式
    const replaceStemImgIdx = ref<number | null>(null)
    const replaceOptionImgCtx = ref<{ label: string; idx: number } | null>(null)
    const replaceAnalysisImgIdx = ref<number | null>(null)

    // 题干图片上传
    const triggerStemImageUpload = () => {
      replaceStemImgIdx.value = null
      const input = document.getElementById('stem-image-input') as HTMLInputElement
      input?.click()
    }

    const triggerReplaceStemImage = (imgIdx: number) => {
      replaceStemImgIdx.value = imgIdx
      const input = document.getElementById('stem-image-input') as HTMLInputElement
      input?.click()
    }
    ...
    const onStemImageSelected = async (evt: Event) => {
      ...
      const replaceIdx = replaceStemImgIdx.value
      replaceStemImgIdx.value = null
      if (!file) return
      ...
      try {
        ...
        if (replaceIdx !== null && editBuf.stem_images[replaceIdx]) {
          // 替换：保留原文件名，确保题干文本中的 [IMG:filename] 标记仍指向新图
          const oldFilename = editBuf.stem_images[replaceIdx].filename
          editBuf.stem_images[replaceIdx] = {
            filename: oldFilename || file.name,
            base64: base64,
            ext: ext,
    ...
    const triggerOptionImageUpload = (label: string) => {
      ...
      replaceOptionImgCtx.value = null
      const input = document.getElementById('option-image-input') as HTMLInputElement
      input?.click()
    }

    const triggerReplaceOptionImage = (label: string, imgIdx: number) => {
      currentOptionLabel = label
      replaceOptionImgCtx.value = { label, idx: imgIdx }
      const input = document.getElementById('option-image-input') as HTMLInputElement
      input?.click()
    }
    ...
    const onOptionImageSelected = async (evt: Event) => {
      const input = evt.target as HTMLInputElement
      const file = input?.files?.[0]
      const replaceCtx = replaceOptionImgCtx.value
      replaceOptionImgCtx.value = null
      ...
      try {
        ...
        if (
          replaceCtx &&
          replaceCtx.label === currentOptionLabel &&
          editBuf.option_images[currentOptionLabel][replaceCtx.idx]
        ) {
          const oldFilename = editBuf.option_images[currentOptionLabel][replaceCtx.idx].filename
          editBuf.option_images[currentOptionLabel][replaceCtx.idx] = {
            filename: oldFilename || file.name,
            base64: base64,
            ext: ext,
          ...
          ElMessage.success(`选项${currentOptionLabel}图片已替换`)
        } else {
    ...
    const triggerAnalysisImageUpload = () => {
      replaceAnalysisImgIdx.value = null
      const input = document.getElementById('analysis-image-input') as HTMLInputElement
      input?.click()
    }

    const triggerReplaceAnalysisImage = (imgIdx: number) => {
      replaceAnalysisImgIdx.value = imgIdx
      const input = document.getElementById('analysis-image-input') as HTMLInputElement
      input?.click()
    }
    ...
    const onAnalysisImageSelected = async (evt: Event) => {
      const input = evt.target as HTMLInputElement
      const file = input?.files?.[0]
      const replaceIdx = replaceAnalysisImgIdx.value
      replaceAnalysisImgIdx.value = null
      if (!file) return
      ...
      try {
        ...
        if (replaceIdx !== null && editBuf.analysis_images[replaceIdx]) {
          const oldFilename = editBuf.analysis_images[replaceIdx].filename
          editBuf.analysis_images[replaceIdx] = {
            filename: oldFilename || file.name,
            base64: base64,
            ext: ext,
            content_type: file.type,
          }
          ElMessage.success('解析图片已替换')
        } else {
    ...
    // 题干 / 选项文本渲染：转义 + 换行 + 表格块（[表格]...[/表格] → <table>）
    // 第二个可选参数：题目对象（含 stem_images / option_images / analysis_images），
    // 用于把 [表格] 块里的 [IMG:filename] 标记替换成 <img>。
    const formatStem = (text: string | null | undefined, q?: any): string => {
      const imagesMap = q ? buildQuestionImagesMap(q) : undefined
      return formatTextWithTables(text, imagesMap)
    }

    // 题干图片卡片网格使用：过滤掉已内联在 [表格] 块里的图片，避免重复显示
    const visibleStemImagesOf = (q: any): any[] => visibleStemImages(q)

    // 表格内嵌入图片的点击放大：v-html 内的 <img> 不会触发 Vue @click，
    // 用事件委托：在外层容器上监听点击，命中 .md-table-img 时打开预览
    const onStemContentClick = (e: MouseEvent) => {
      const target = e.target as HTMLElement | null
      if (!target) return
      if (target.tagName === 'IMG' && target.classList.contains('md-table-img')) {
        const src = (target as HTMLImageElement).src
        if (src) previewImage(src)
      }
    }

    // 编辑模式下：列出 stem 文本中通过 [IMG:filename] 标记内联的表格图片
    // 返回 [{ filename, src }]，src 为 data URL；找不到 base64 时跳过
    const tableImagesInStem = (q: any): Array<{ filename: string; src: string }> => {
      if (!q) return []
      const filenames = extractImageMarkerFilenames(q.stem)
      if (filenames.length === 0) return []
      const map = buildQuestionImagesMap(q)
      const out: Array<{ filename: string; src: string }> = []
      for (const fn of filenames) {
        const src = map[fn]
        if (src) out.push({ filename: fn, src })
      }
      return out
    }

    // 编辑模式删除：从 stem 文本中剔除 [IMG:filename]，并从 stem_images 中移除对应条目
    const deleteTableImageInItem = (item: any, filename: string) => {
      if (!item || !filename) return
      item.stem = removeImageMarker(item.stem || '', filename)
      if (Array.isArray(item.stem_images)) {
        item.stem_images = item.stem_images.filter((x: any) => x?.filename !== filename)
      }
      // option_images / analysis_images 中如果也有同名项，一并清掉（防御性）
      if (item.option_images && typeof item.option_images === 'object') {
        for (const k of Object.keys(item.option_images)) {
          item.option_images[k] = (item.option_images[k] || []).filter((x: any) => x?.filename !== filename)
        }
      }
      if (Array.isArray(item.analysis_images)) {
        item.analysis_images = item.analysis_images.filter((x: any) => x?.filename !== filename)
      }
      ElMessage.success('已删除表格内图片')
    }

    const processAnalysisText = (text: string | null | undefined, q?: any): string => {
      ...
      let cleaned = text
        ...
        .replace(/^\s*\*\*\s*$/gm, '')
        .replace(/\*\*(?=\s*$)/gm, '')
      // 先把 [表格]...[/表格] 替换为 <table> HTML 占位符（renderTableBlocks 会用私有区字符包裹），
      // 再剥掉占位符让 markdown 渲染剩余文本——占位符里是裸 HTML，markdown 渲染器会原样保留。
      // 第二个参数：题目对象，用于把表格里 [IMG:filename] 标记还原为 <img>
      const imagesMap = q ? buildQuestionImagesMap(q) : undefined
      const withTables = renderTableBlocks(cleaned, imagesMap)
        .split('\uE000').join('')
        .split('\uE001').join('')
      return renderMarkdown(withTables)
    }
    ...
    // 试卷列表管理函数
    const loadPaperList = async (force: boolean = false) => {
      loadingPaperList.value = true
      try {
        // 管理端获取全部试卷（包含可见性状态）
        // force=true 时旁路服务端 60s 缓存，确保新生成 / 删除的试卷立即同步
        const url = `${MCQ_BASE_URL}/papers/list_open?visible_only=false${force ? '&force=true' : ''}`
    ...
    // 新增一道空白题目（在试卷预览/编辑对话框内手动添加）
    const addNewPaperItem = () => {
      const newItem: any = {
        qtype: 'single',
        stem: '',
        options: { A: '', B: '', C: '', D: '' },
      ...
      uploadedPaperItems.value.push(newItem)
      const newIdx = uploadedPaperItems.value.length - 1
      editingPaperItemIdx.value = newIdx
      // 滚动到新题位置
      nextTick(() => {
        const container = document.querySelector('.el-dialog__body [style*="max-height: 500px"]') as HTMLElement | null
        if (container) container.scrollTop = container.scrollHeight
      })
      ElMessage.success('已新增一道题目，请在下方编辑')
    }
    ...
    const deleteUploadedItemImage = (itemIdx: number, type: 'stem' | 'option' | 'analysis', imgIdx: number, optionKey?: string, target: 'paper' | 'review' = 'paper') => {
      ...
      if (type === 'stem') {
        if (item.stem_images && item.stem_images[imgIdx]) {
          const removedFn = item.stem_images[imgIdx]?.filename
          item.stem_images.splice(imgIdx, 1)
          // 同步剔除题干文本里对应的 [IMG:filename] 标记，避免出现"无图却有引用"的孤儿
          if (removedFn && typeof item.stem === 'string') {
            item.stem = removeImageMarker(item.stem, removedFn)
          }
    ...
    const handleItemImageUpload = async (evt: Event) => {
      ...
      try {
        ...
        reader.onload = () => {
          ...
          // 替换模式：保留原文件名，避免题干 [IMG:filename] 等引用失效
          const tryReplace = (arr: any[]): boolean => {
            if (replaceIdx === undefined || !arr || !arr[replaceIdx]) return false
            const oldFilename = arr[replaceIdx]?.filename
            if (oldFilename) imgData.filename = oldFilename
            arr[replaceIdx] = imgData
            return true
          }
          ...
          if (type === 'stem') {
            ...
            replaced = tryReplace(item.stem_images)
            if (!replaced) item.stem_images.push(imgData)
          } else if (type === 'option' && optionKey) {
            if (!item.option_images) item.option_images = {}
            if (!item.option_images[optionKey]) item.option_images[optionKey] = []
            replaced = tryReplace(item.option_images[optionKey])
            if (!replaced) item.option_images[optionKey].push(imgData)
          } else if (type === 'analysis') {
            if (!item.analysis_images) item.analysis_images = []
            replaced = tryReplace(item.analysis_images)
            if (!replaced) item.analysis_images.push(imgData)
          }

          updateItemHasImages(item)
          ElMessage.success(replaced ? '图片已替换' : '图片已添加')
        }
        reader.readAsDataURL(file)
      } catch (e: any) {
        ElMessage.error('图片读取失败：' + (e?.message || e))
    ...
    const loadPaperForEdit = async () => {
      ...
      try {
        ...
        uploadedJudgeScore.value = scoreConfig.judge || 1
    ...
    const saveUploadedPaper = async () => {
      ...
      try {
        ...
        const scoreConfig: any = {
          ...
          judge: uploadedJudgeScore.value,
        ...
        // 刷新试卷列表（force=true 旁路服务端缓存）
        await loadPaperList(true)
    ...
    return {
      ...
      processAnalysisText,
      formatStem, visibleStemImagesOf, onStemContentClick, tableImagesInStem, deleteTableImageInItem, extractImageMarkerFilenames,
      // 解析Tab切换相关
...
:deep(.md-table img.md-table-img:hover) {
  transform: scale(1.02);
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.12);
}
/* 题干 v-html 容器：让图片有手型光标提示 */
.stem-html { display: inline; }
.stem-html :deep(.md-table-img) { cursor: zoom-in; }
/* 试卷管理 tab：使表格能在题干 v-html 容器内正确渲染 */
.paper-q-num { margin-right: 4px; font-weight: 500; }
.paper-stem-html { display: inline-block; vertical-align: top; max-width: 100%; }
.paper-stem-html :deep(.md-table) { display: table; }
/* 编辑模式下"表格图片"的删除卡片 */
.table-img-edit-grid {
  display: flex;
  flex-wrap: wrap;
...
.editable-image-wrapper .img-replace-btn {
  position: absolute;
  bottom: -8px;
  right: -8px;
  width: 20px !important;
